...
./groupsio --groupsio-group-name='chipsalliance+announce' --groupsio-save-archives=1 --groupsio-debug=0 --groupsio-es-url="${ESURL}" --groupsio-email="`cat ./secrets/email.encrypted.secret`" --groupsio-password="`cat ./secrets/password.encrypted.secret`" --groupsio-stream="${STREAM}" $*
